...
contract AaveV3AdapterTest is Test {
    ...
    function setUp() public {
        ...
        adapter = new AaveV3Adapter(address(pool), address(this));
    ...
    // ============ test_deploy_second_deploy_proportional_shares ============

    function test_deploy_second_deploy_proportional_shares() public {
        uint256 shares1 = adapter.deploy(address(token), DEPLOY_AMOUNT);

        // Second deploy at same ratio should also produce 1:1 shares (no yield injected)
    ...
    function test_getAPY_returns_value() public view {
        assertEq(adapter.getAPY(address(token)), 500);
    }
    ...
    function test_recall_insufficient_shares_reverts() public {
        vm.expectRevert("AaveV3Adapter: insufficient shares");
    ...
    function test_deploy_only_router_reverts() public {
        ...
        vm.prank(nonRouter);
        vm.expectRevert("AaveV3Adapter: only router");
