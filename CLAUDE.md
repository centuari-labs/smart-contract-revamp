...
- **Manual post-redeploy step — `HUB_START_BLOCK` (indexer-v3).** `sync-to-services.sh` does NOT write `HUB_START_BLOCK`: the deployment summary JSON carries no deploy-block, so there is nothing to source it from. After every redeploy to a brownfield chain, set `HUB_START_BLOCK` in `indexer-v3/.env` to the deployment block (e.g. the block of the first `run-all.sh` tx, or the chain head captured at deploy time) and reset the indexer's `block_cursor` — otherwise the indexer rescans from genesis (millions of blocks) and never catches the head. The only env key the automatic sync can't cover. (Faucet keys ARE synced: `gen_backend_env` emits `FAUCET_ADDRESS_<chainid>` + `FAUCET_TOKENS_<chainid>` from the deploy JSON's `faucetTokensRaw`.)
