...
#### A4. Medium-severity polish (9 items; #26 deferred)

| # | Severity | Description |
|---|---|---|
| 8 | ✅ DONE 2026-05-15 | `POST /faucet/request-tokens` now runs `@UseGuards(AuthGuard)` and binds `dto.recipientAddress` to the authenticated wallet via `@Wallet()` (case-insensitive equality, `ForbiddenException` on mismatch) — see [`faucet.controller.ts`](../../backend-v2/src/faucet/faucet.controller.ts). Integration coverage in [`__test__/integration/faucet-request-tokens.integration.test.ts`](../../backend-v2/src/__test__/integration/faucet-request-tokens.integration.test.ts) for the 401 (no header / empty Bearer) / 200 (match, incl. uppercase) / 403 (mismatch) paths. Mainnet full disable still tracked separately by D6. |
| 9 | ✅ DONE 2026-05-15 | Deleted duplicate `src/lib/fee-calculations.ts`; both [`centuari-borrow-dialog.tsx`](../../frontend-revamp/src/components/centuari-borrow-dialog.tsx) and [`centuari-lend-dialog.tsx`](../../frontend-revamp/src/components/centuari-lend-dialog.tsx) now consume the canonical [`calculateOrderFees`](../../frontend-revamp/src/lib/fee-utils.ts) with the `"market"` literal (both dialogs call `submitMarket`, so taker rate is correct — the audit's "limit dialogs" framing didn't match the code). Vitest coverage in [`fee-utils.test.ts`](../../frontend-revamp/src/lib/__tests__/fee-utils.test.ts) for maker vs taker discrimination and the settlement-fee cap. |
| 10 | ✅ DONE 2026-05-15 | Dropped the `?? wallets.find(privy)` silent fallback in [`use-deposit.ts`](../../frontend-revamp/src/hooks/use-deposit.ts); a missing external wallet now throws a new `WalletNotConnectedError` from [`errors.ts`](../../frontend-revamp/src/lib/errors.ts). Both deposit consumers ([`centuari-deposit-dialog.tsx`](../../frontend-revamp/src/components/centuari-deposit-dialog.tsx), [`centuari-lend-dialog.tsx`](../../frontend-revamp/src/components/centuari-lend-dialog.tsx)) catch the typed error and toast "Please reconnect your external wallet". Sister `use-network-switch.ts` fallback intentionally out of scope. Coverage in [`use-deposit.test.ts`](../../frontend-revamp/src/hooks/__tests__/use-deposit.test.ts) for both external-wallet-present and external-wallet-missing paths. |
| 11 | ✅ DONE 2026-05-15 | Migrated 4 paginated endpoints (`getMyPositions`, `getMyAssets`, `getOrderHistory`, `getTransactionHistory`) in [`api.ts`](../../frontend-revamp/src/lib/api.ts) off raw `fetch` onto a new [`apiClientPaginated`](../../frontend-revamp/src/lib/api-client.ts) helper (returns `{ data, meta }`) — same module also gains a `params` query-string option on `apiClient`. Coverage in [`api-client.test.ts`](../../frontend-revamp/src/lib/__tests__/api-client.test.ts) and [`api-contract.test.ts`](../../frontend-revamp/src/lib/__tests__/api-contract.test.ts). |
...
| 17 | ✅ DONE 2026-05-15 | Docker / CI hardening bundle landed — [`.dockerignore`](../../frontend-revamp/.dockerignore) excludes `.env*` / `docs/` / `e2e/` / `playwright-report/` / `coverage/` / `test-results/`; [`deploy.yml`](../../frontend-revamp/.github/workflows/deploy.yml) fails the prod build on `NEXT_PUBLIC_USE_MOCK=true` for `main` / `testnet`; runtime stage in [`Dockerfile`](../../frontend-revamp/Dockerfile) runs as `USER node` after `chown -R node:node /app`, and both `node:22-alpine` stages digest-pinned to `sha256:968df39aedcea65eeb078fb336ed7191baf48f972b4479711397108be0966920`; wallet picker shows `info.rdns` under `info.name` in [`centuari-connect-wallet.tsx`](../../frontend-revamp/src/components/centuari-connect-wallet.tsx); E2E `TEST_WALLET` deduped into [`e2e/helpers/test-wallet.ts`](../../frontend-revamp/e2e/helpers/test-wallet.ts) on the Anvil dev key #0 (`0xf39Fd6…2266`, coordinated with B1). |
| 19 | ✅ DONE 2026-05-15 | `mapStatus` in [`positions-adapter.api.ts`](../../frontend-revamp/src/lib/positions-adapter.api.ts) now logs (`console.error`) and returns `"UNKNOWN"` on unrecognised backend status — the silent `"OPEN"` fallback is gone. `PositionStatus` union extended in [`types/positions.ts`](../../frontend-revamp/src/types/positions.ts), the `statusColors` map in [`position-section.tsx`](../../frontend-revamp/src/components/market/position-section.tsx) renders a neutral muted badge for `UNKNOWN`, and the narrower duplicates in [`centuari-table.tsx`](../../frontend-revamp/src/components/centuari-table.tsx) were widened to reuse `PositionStatus`. Coverage in [`positions-adapter.api.test.ts`](../../frontend-revamp/src/lib/__tests__/positions-adapter.api.test.ts). |
| 21 | ✅ DONE 2026-05-15 | Dropped silent `decimals = 6` default in [`use-orderbook.ts`](../../frontend-revamp/src/hooks/use-orderbook.ts) and [`use-recent-trades.ts`](../../frontend-revamp/src/hooks/use-recent-trades.ts); both hooks now require `decimals: number \| undefined` and skip the WS subscription when it is `undefined`. The two form hooks ([`use-borrow-form.ts`](../../frontend-revamp/src/hooks/use-borrow-form.ts), [`use-lend-form.ts`](../../frontend-revamp/src/hooks/use-lend-form.ts)) thread `decimals` from `useMarketDetail`. Hook tests updated in [`use-orderbook.test.ts`](../../frontend-revamp/src/hooks/__tests__/use-orderbook.test.ts) and [`use-recent-trades.test.ts`](../../frontend-revamp/src/hooks/__tests__/use-recent-trades.test.ts) with new "decimals undefined → no subscription" cases. |
| 24 | ✅ DONE 2026-05-15 | Replaced silent `return` validation gates with `sonner` `toast.error` guards in [`use-borrow-form.ts`](../../frontend-revamp/src/hooks/use-borrow-form.ts) (`handleLimitSubmit` + `handleMarketSubmit`, 5 amount/quota/HF gates each + a new `resolvedMarketId` maturity guard above the DTO build) and [`centuari-withdraw-dialog.tsx`](../../frontend-revamp/src/components/centuari-withdraw-dialog.tsx) (`handleWithdraw` defence-in-depth on top of the existing `disabled` state). Coverage in [`use-borrow-form.test.ts`](../../frontend-revamp/src/hooks/__tests__/use-borrow-form.test.ts) (`vi.mock("sonner", ...)` + per-handler amount-zero / exceeds-quota toast assertions). Follow-up: filter `availableMaturities` to backend-confirmed entries upstream — tracked separately. |
...
| # | Status | Description |
|---|---|---|
| B1 | 🟡 PARTIAL 2026-05-15 | Unblock 8 skipped E2E collateral scenarios via Path 1 (capture real Privy session into `storageState`). **Infra shipped:** [`frontend-revamp/e2e/capture-privy-session.ts`](../../frontend-revamp/e2e/capture-privy-session.ts) (headless SIWE — injects an EIP-6963 mock provider backed by a hardcoded test private key, lets Privy + wagmi auto-detect the announced provider and run SIWE on page load, persists session to `e2e/.auth/privy.json`, gitignored); [`frontend-revamp/e2e/auth.setup.ts`](../../frontend-revamp/e2e/auth.setup.ts) (validates session before each `pnpm test:e2e` run); [`playwright.config.ts`](../../frontend-revamp/playwright.config.ts) projects (`setup`, `chromium`, `capture`); rewritten [`collateral-toggle.spec.ts`](../../frontend-revamp/e2e/collateral-toggle.spec.ts) that drops `preflightOrSkip` / `setupAuth` Privy stubs / `mockPrivyEndpoints` and adopts `test.use({ storageState })`. `WALLET_ADDRESS` is the viem-canonical address `0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266` (Anvil dev key #0). Runbook: [`frontend-revamp/e2e/.auth/README.md`](../../frontend-revamp/e2e/.auth/README.md). **Remaining:** run `pnpm test:e2e:capture` against a running dev server, then `pnpm test:e2e` and confirm all 8 scenarios pass — flip this row to ✅ DONE after that. Paths 2 (mock-shape match) and 3 (`useAuthToken.ts` window flag) remain documented in the spec leading comment for future reference. CI E2E intentionally deferred (needs ephemeral pg/redis/nats + RPC + session-refresh story). |
...
| C6 | NOT STARTED | Architectural / pre-mainnet | **Retire `HubDepositor.payout`; route `/withdraw` through `WithdrawalRegistry`.** Locks the launch architecture: deposits via HubDepositor, withdrawals via WithdrawalRegistry. Requires (a) a real HF-aware `RiskModule` to replace Phase 1 `RiskModuleStub` — needs Centuari to expose per-user debt aggregation (currently it doesn't; see [`RiskModuleStub.sol:12-23`](../src/core/risk/RiskModuleStub.sol)); (b) new operator-callable variant `requestWithdrawalFor(user, asset, amount, targetChainId)` on `WithdrawalRegistry` since user principle is "user signs deposits only, backend signs everything else"; (c) backend rewire of [`withdraw.service.ts`](../../backend-v2/src/withdraw/withdraw.service.ts) to submit `requestWithdrawalFor` then `authorize(requestId)` in sequence (both operator-only, both same handler); (d) [`apply-withdraw.ts`](../../backend-v2/src/core/on-chain-state/apply-withdraw.ts) (built in C3 Phase 2) survives unchanged — `BalanceLedger.Debited` fires whether the debit comes from `payout` (today) or `payoutDirect` via `authorize` (future). Phase 1 routing through WithdrawalRegistry would regress collateral withdrawals because the stub's `canWithdraw` is amount-blind and fail-closes any flagged asset. |
| C7 | NOT STARTED | Architectural / async / maintainability | **Lift per-event SQL mutations into `@centuari-labs/on-chain-effects`.** Today the same `INSERT INTO user_balance ... ON CONFLICT ... DO UPDATE SET available = user_balance.available + EXCLUDED.available` SQL lives in two places, byte-identical, maintained by code-review discipline ("mirror the indexer tail byte-for-byte" — see [`apply-repay.ts:24-29`](../../backend-v2/src/core/on-chain-state/apply-repay.ts)): backend-v2 [`apply-withdraw.ts`](../../backend-v2/src/core/on-chain-state/apply-withdraw.ts) / [`apply-repay.ts`](../../backend-v2/src/core/on-chain-state/apply-repay.ts) / [`apply-withdraw-lend.ts`](../../backend-v2/src/core/on-chain-state/apply-withdraw-lend.ts) AND indexer-v3 [`balance-ledger.processor.ts`](../../indexer-v3/src/processors/balance-ledger.processor.ts) / [`centuari.processor.ts`](../../indexer-v3/src/processors/centuari.processor.ts). Drift between eager and tail writers silently breaks idempotency. **Fix:** add tx-agnostic per-event mutation functions to the package (`applyDebitedMutation(tx, decoded, stamp)`, `applyCreditedMutation(...)`, `applyRepaidMutation(...)`, `applyLendPositionWithdrawnMutation(...)`, `applyCollateralFlagSetMutation(...)`, etc.) so the same SQL is executed by both paths by construction, not by review. Caller owns the `pg.PoolClient` so both transactional scopes work — eager (own per-event tx via `pool.connect` + `BEGIN/COMMIT`) and tail (shared per-block tx, `ctx.client` already inside `BEGIN`). Backend `apply-*.ts` shrinks to "scan receipt + loop + call shared mutation"; indexer processors shrink to "decode log + call shared mutation". Multi-service refactor — per project memory "split cross-service changes into per-service phases" ship per service: (a) extend `@centuari-labs/on-chain-effects` with mutations + tests + republish; (b) swap backend-v2 apply-*.ts to call new mutations; (c) swap indexer-v3 processors to call new mutations; (d) sweeper-bot and settlement-engine adopt on their next refactor. **NOT pre-mainnet blocking** — current code works; this is drift-prevention + developer ergonomics. |
...
| **6. Medium polish** ✅ DONE 2026-05-15 | A4 (9 items shipped; #26 deferred) | Pre-launch cleanup — can run async with phase 5 |
...
3. **A4 #26 — SubmitProofDialog.** ✅ DECIDED 2026-05-15 — **deferred past launch** (leaderboard not in hub-only launch scope; see "Async / post-launch" row in §4). Options preserved for the post-launch sprint:
