...
| **P3** | **Settlement Engine** | ⚪ NOT STARTED (unblocked) | Encode the borrower's pending flag requests into `MatchData.collateralAssets` per borrower in the `Settlement.settleMatches` ABI; clear the queue on settlement success. | P1b-explicit ABI ✅ + P2 ✅ |
...
- **Phase status as of 2026-04-28:**
  ...
  - **P3** → ⚪ **NOT STARTED** (unblocked). Scope: settlement engine encodes the borrower's pending unfulfilled flag requests into `MatchData.collateralAssets` per borrower; clears the queue on settlement success.
...
**Next actionable phase:** **P3 (settlement engine)** — its upstream gate (P2 ABI) is now met. Scope: encode `MatchData.collateralAssets` per borrower in `Settlement.settleMatches`, dedupe within a batch, and clear the backend's `pending_collateral_flags` queue on settlement success. P6 portfolio unflag UI unlocks once P3 closes out the pending-flag-queue branch of P4. M7 (sweeper-bot) is independent and unblocked.
...
### P3 — settlement engine (⚪ NOT STARTED — unblocked, P2 ABI met)

- `settlement-engine/src/settlement/smartContract.ts` — read `borrowerCollateralAssets` off each parsed match (already present on the Redis-stream payload thanks to P2), dedupe within a settlement batch per borrower, encode into `MatchData.collateralAssets` for `Settlement.settleMatches`, and clear the backend's `pending_collateral_flags` queue rows for that borrower on settlement success.
- Receipt parsing already carries the data needed to stamp via `applyOnChainEffect` from `@centuari-labs/on-chain-effects` ^0.2.0 (per phase-A A2). No new helper required.
