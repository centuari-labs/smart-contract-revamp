...
| **P1b** | **Smart Contracts (deferred to M2/M4 start)** | 🔓 UNBLOCKED (M2 ✅ M4 ✅ — ready to start) | Extend `Settlement.MatchData` with `collateralAssets[]`; add auto-flag loop inside `Settlement._processMatch`; add per-user open-market set + `hasOutstandingDebt` view in `Centuari.sol`; add auto-unflag loop inside `Centuari.repay`. `WithdrawalRegistry.sol` with `riskModule.canWithdraw` gate already landed in M4. | P1a ✅ + M2 ✅ + M4 ✅ |
...
**Current module state per doc (as of 2026-04-12):** M1 🟢 DONE, M1b 🟢 DONE (both landed 2026-04-09). M2–M3 🟢 DONE (landed as part of M1-M3). M4 🟢 DONE (landed 2026-04-12; 339 tests passing). M5–M10 not started; M5 is now **unblocked** and is the next priority. This plan file is the durable reference; phases land over time.
