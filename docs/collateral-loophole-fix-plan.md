...
| **P4** | **Backend** | 🔒 BLOCKED | Delete old `/internal/collateral` relay. Add two app-user endpoints backed by Privy JWT + 5/user/24h Redis rate limit: `POST /collateral/flag { asset }` (enqueues an unfulfilled flag request when the user has a pending borrow **or** calls `CollateralManager.flagFor` directly with the protocol settlement key for users with no pending borrow) and `POST /collateral/unflag { asset }` (calls `CollateralManager.unflagFor` with the protocol settlement key). Both flows call `applyOnChainEffect` **inline** in the handler (backend-direct — **no frontend confirmation endpoint**; see phase-1 §C10.1). Imports the helper from `@centuari/indexer-v3/shared/apply-on-chain-effect` via pnpm workspace (phase-1 §C10.3). | P1a + indexer-v3 existing (M8 start) |
| **P5** | **Indexer** | 🔒 BLOCKED | Add `CollateralFlagSet` processor writing `user_balance.used_as_collateral` + `flagged_at` with C10 idempotency stamps. Tail treats rows already stamped with a matching `applied_by_tx_hash` as idempotent no-ops; its direct role is the safety net for events the eager path missed (service crash mid-handler) and paths with no eager writer. Per P1b-explicit, `Centuari.repay` no longer emits `CollateralFlagSet`, so no tail-only flag events exist from that path. | P1a events + M8 start |
