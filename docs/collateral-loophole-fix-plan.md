...
| **P2** | **Matching Engine** | ⚪ NOT STARTED | Add `collateralAssets: string[]` to borrow-order output so the settlement engine can pull the user's pending unfulfilled flag requests from the queue and attach them to each match. | P1b-explicit ABI ✅ + M9 backend collateral ✅ |
| **P3** | **Settlement Engine** | 🔒 BLOCKED on P2 | Encode the borrower's pending flag requests into `MatchData.collateralAssets` per borrower in the `Settlement.settleMatches` ABI; clear the queue on settlement success. | P1b-explicit ABI ✅ + P2 |
...
- **Phase status as of 2026-04-27:**
  - **P2** → ⚪ **NOT STARTED** (unblocked). All upstream gates met (P1b-explicit ABI ✅, backend collateral landed under M9 ✅). Scope: add `collateralAssets: string[]` to borrow-order output so the settlement engine can attach pending user flag requests to each match.
  - **P3** → 🔒 BLOCKED on P2 only (no longer blocked on M9 start). Scope: settlement engine encodes the borrower's pending unfulfilled flag requests into `MatchData.collateralAssets` per borrower; clears the queue on settlement success.
  - **P4 backend** → ✅ **DONE** (except the pending-flag-queue branch, which lands with P2/P3). `backend-v2/src/collateral/` exposes `POST /collateral/{flag,unflag}` driven by `CollateralManager.{flagFor,unflagFor}` with `applyOnChainEffect` C10 stamping. Legacy `/internal/collateral` relay deleted.
  ...
  - **P6 frontend** → 🟡 **IN PROGRESS**. Borrow-form multi-select landed in `centuari-borrow-dialog.tsx`. Portfolio unflag UI + countdown + `use-unflag-collateral.ts` hook + e2e specs still pending.

**Next actionable phase:** **P2 (matching engine)** — the only collateral-loophole phase whose upstream gates are all met. P3 unlocks once P2 ships; P6 portfolio unflag UI unlocks once P2+P3 close out the pending-flag-queue branch of P4. M7 (sweeper-bot) is independent and unblocked.
