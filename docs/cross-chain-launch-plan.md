...
| A5 — Migrate backend-v2 onto shared on-chain-state schema | 🟡 PARTIAL — `/portfolio/*` reads on shared schema via `OnChainStateRepository`. Outstanding: legacy `portfolio` + `lend_positions` reads (hub-only Track C3, pre-mainnet blocker); legacy `markets` UUID reads in `/market` + repay + withdraw-lend-position + `/orders/*` + workers (hub-only Track C4, matching-engine coupling). A6 (DROP TABLE) cannot start until C3 + C4 land; C5 (drop dead tables) is independent and safe today. |
